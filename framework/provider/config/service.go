...
// 读取某个配置文件
func (conf *LsfConfig) loadConfigFile(folder string, file string) error {
	conf.lock.Lock()
...
// 删除文件的操作
func (conf *LsfConfig) removeConfigFile(folder string, file string) error {
	conf.lock.Lock()
	defer conf.lock.Unlock()
	s := strings.Split(file, ".")
	// 只有yaml或者yml后缀才执行
...
// NewLsfConfig 初始化Config方法
func NewLsfConfig(params ...interface{}) (interface{}, error) {
...
// 通过path来获取某个配置项
func (conf *LsfConfig) find(key string) interface{} {
	conf.lock.RLock()
